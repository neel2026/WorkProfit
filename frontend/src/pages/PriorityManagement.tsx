...
export default function PriorityManagement() {
    ...
    useEffect(() => {
        const fetchPriorities = async () => {
            try {
                const res = await apiClient.get<string[]>('/priorities/');
                setPriorities(res.data.map(code => ({
                    id: code,
                    name: code.toLowerCase().replace(/\b\w/g, c => c.toUpperCase())
                })));
            } catch (err) {
        ...
        <Layout>
            <div className="flex flex-col gap-6 w-full">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 italic">
                    Default priorities are system-defined.
                </p>
            </div>
        </Layout>
    );
